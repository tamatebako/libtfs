...
### Added
- Third-party readiness for the C ABI (item 24): `TEBAKO_FS_ABI_VERSION` (starts at 1) and `tebako_fs_abi_version()` let adapters feature-detect the ABI at load time, independent of the release version; shared-library builds `libtfs.dylib` (macOS) and `libtfs.so` (Linux) alongside `libtfs.a`, exporting ONLY the `tebako_*` C ABI (link-time gate: `cmake/libtfs.exports` exported-symbols list on macOS, `cmake/libtfs.map` version script on ELF; Windows `.dll`/`.def` deferred to the release pipeline); `libtfs.pc` pkg-config for shared (`-ltfs`) and static (`--static`, transitive closure via Requires/Libs.private) consumption, installed next to the existing CMake package config; `docs/EMBEDDING.md` — the embedding guide (incbin vs tebakofs-append, mount/multi-mount, read-only model + EROFS, single-mutex threading, error model, C + Python-cffi adapter templates, per-backend license statement); `adapter_smoke_c` ctest case — a minimal C program compiled with pkg-config flags against the shared library, binding-checked (otool/readelf) and run against a ZIP fixture.
...
### Fixed
- The release `tebakofs` binaries mount SquashFS again on POSIX platforms: v0.13.0 shipped the tool from the consumer-library build configuration (`WITH_SQUASHFS=OFF`, which keeps LGPL squashfs-tools-ng out of consumer packages), so the binary silently lacked the backend (found by the tebako-rs M4 golden tests probing the release oracle). The release pipeline now builds `tebakofs` from a second, squashfs-enabled configure of the same tree and smoke-checks the capability on every POSIX leg; consumer `libtfs` packages stay `WITH_SQUASHFS=OFF`. Builds without the backend (Windows, where it is POSIX-only) now report the gap loudly instead of a bare "unsupported format": `tebakofs` prints a dedicated SquashFS-not-built-in message (non-zero exit) and the C API fails mounts of SquashFS images with `ENOTSUP` (magic-verified, so missing/corrupt files still yield `EINVAL`). `BackendFactory::squashfs_supported()` plus always-on SquashFS magic/extension detection (plain byte checks, no squashfs-tools-ng) let callers tell "unknown format" and "capability not built in" apart.
